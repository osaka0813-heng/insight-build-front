...
const DEFAULT_FOCUS = '过去24小时内，寻找最可能改变现有世界进程判断的重要跨领域信号，重点关注AI、能源、地缘政治、宏观经济与资本。';
...
export function EditorialConsoleScreen({ onBack, onPublished }: Props) {
  ...
  const [overrideReason, setOverrideReason] = useState('人工审核确认该内容值得作为持续洞察更新发布。');
  ...
  const forceWrite = async () => {
    ...
    try {
      ...
      setWriteState('error');
      setError(await recordError('write', forceError));
    }
  };

  const publish = async (action: 'approve' | 'reject', override = false) => {
    if (!writerDraft) return;

    if (action === 'approve' && writerMissingFields.length > 0) {
      setError(
    ...
    if (action === 'approve' && preflight?.ok !== true) {
    ...
    setMessage(undefined);
    try {
      const result = await reviewWriterDraft(writerDraft, action, publishToken, {
        override,
  ...
  return (
    <SafeAreaView style={styles.root}>
      <View style={styles.header}>
        ...
        <Text style={styles.eyebrow}>BUILD012.4 DELTA</Text>
        <Text style={styles.title}>AI 编辑台</Text>
        <Text style={styles.subtitle}>发布前验证、自动备份、防重复发布与故障诊断。</Text>
      </View>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <Section title="今日研究">
          ...
          {writeState === 'blocked' && analyzedDraft ? (
            <Pressable
              disabled={isBusy}
              onPress={() => void forceWrite()}
              style={styles.secondaryButton}
            >
              <Text style={styles.secondaryText}>生成覆盖草稿</Text>
            </Pressable>
          ) : null}
          {restoredAt ? (
        ...
        {writerDraft && writerCopy ? (
          <Section title="Writer 草稿">
            ...
            <Text style={styles.meta}>世界进程：{writerDraft.matchedProcessId || '未匹配'}</Text>
            ...
            {!canNormalApprove ? (
              <TextInput value={overrideReason} onChangeText={setOverrideReason} multiline style={[styles.input, styles.reason]} placeholder="人工覆盖理由" />
            ) : null}
            <Pressable
              disabled={!canNormalApprove || publishState === 'running'}
              onPress={() => void publish('approve', false)}
              style={[
                styles.primaryButton,
                !canNormalApprove && styles.disabled,
              ]}
            >
            ...
            {writerDraft?.qualityChecks.publishThresholdMet !== true ? (
              <Pressable
                ...
                onPress={() => void publish('approve', true)}
                style={[
                  styles.overrideButton,
                  (!writerDraftComplete || preflight?.ok !== true) &&
                    styles.disabled,
                ]}
              >
                <Text style={styles.overrideText}>人工覆盖发布</Text>
              </Pressable>
            ) : null}
            <Pressable disabled={publishState === 'running'} onPress={() => void publish('reject')} style={styles.rejectButton}><Text style={styles.rejectText}>拒绝草稿</Text></Pressable>
          </Section>
        ) : null}
